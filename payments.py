    ...
    async def create(self, order) -> Invoice:
        ...
        if not addr and hdwallet.ready():
            try:
                ...
                raise
        if not addr:
            # No fallback to EVM_ADDRESS. That is the shop's main wallet — the
            # place swept funds are collected *to* — so paying into it would
            # mix customer payments with already-collected money and leave the
            # order with nothing to match against. Refusing keeps the other
            # rails selling while this is fixed.
            log.error("no deposit address available (%s) — refusing checkout",
                      hdwallet.problem())
            raise RuntimeError(
